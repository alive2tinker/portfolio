<article class="group relative flex flex-col items-start">
    <h2 class="text-base font-semibold tracking-tight text-zinc-800 dark:text-zinc-100">
        ...
        <a href="{{ $experience->organization_link }}"><span
                class="absolute -inset-y-6 -inset-x-4 z-20 sm:-inset-x-6 sm:rounded-2xl"></span><span
                class="relative z-10">{{ $experience->position }} {{ __('at') }} {{ $experience->organization_name }}</span></a></h2>
    <time
        class="relative z-10 order-first mb-3 flex items-center text-sm text-zinc-400 dark:text-zinc-500 pl-3.5"
        datetime="{{ $experience->start_date->format('Y-m-d') }}"><span class="absolute inset-y-0 left-0 flex items-center"
                                    aria-hidden="true"><span
                class="h-4 w-0.5 rounded-full bg-zinc-200 dark:bg-zinc-500"></span></span>{{ $experience->start_date->format('Y-m-d') }} - {{ $experience->end_date ? $experience->end_date->format('Y-m-d') : __('Present')}}
    </time>
    <p class="relative z-10 mt-2 text-sm text-zinc-600 dark:text-zinc-400">{{ $experience->duties }}</p>
    <div aria-hidden="true" class="relative z-10 mt-4 flex items-center text-sm font-medium text-teal-500">
        {{ __('More About Organization') }}
